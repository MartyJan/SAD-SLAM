...
class Mapper(object):
    ...
    def optimize_map(self, num_joint_iters, lr_factor, idx, cur_gt_color, cur_gt_depth, gt_cur_c2w, keyframe_dict, keyframe_list, cur_c2w):
        ...
        for joint_iter in range(num_joint_iters):
            ...
            # if self.SA and self.stage!= 'color' and self.stage!= 'coarse':

            #     if self.stage == 'coarse': 
            #         model = self.decoders.coarse_decoder
            #         para = self.decoders.coarse_decoder.parameters()
            #     elif self.stage == 'middle': 
            #         model = self.decoders.middle_decoder
            #         para = self.decoders.middle_decoder.parameters()
            #     elif self.stage == 'fine': 
            #         model = self.decoders.fine_decoder
            #         para = self.decoders.fine_decoder.parameters()

            #     lr = 0.00003
            #     optim = torch.optim.Adam(para, lr=lr)
            #     # batch_rays_d = torch.unsqueeze(batch_rays_d[i],0).clone().detach().requires_grad_(True)
            #     # batch_rays_o = torch.unsqueeze(batch_rays_o[i],0).clone().detach().requires_grad_(True)
            #     batch_rays_d = batch_rays_d.clone().detach().requires_grad_(True)
            #     batch_rays_o = batch_rays_o.clone().detach().requires_grad_(True)
            ...
            #         # batch_rays_d = torch.unsqueeze(batch_rays_d[i],0)
            #         # batch_rays_o = torch.unsqueeze(batch_rays_o[i],0)
            #         raw, raw_out = self.renderer.render_batch_ray(c, self.decoders, batch_rays_d,
            #                                     batch_rays_o, device, self.stage,
            #                                     gt_depth=None if self.coarse_mapper else batch_gt_depth, sa = True)
                  
            #         logits = torch.cat([raw, raw_out], dim=1)
            #         occ = torch.cat([torch.ones((1, raw.size()[1]), dtype=torch.float32)*0.5, torch.ones((1, raw_out.size()[1]), dtype=torch.float32)], dim=1).to(device)
            #         # occ = torch.cat([torch.ones((raw.size()[0], raw.size()[1]), dtype=torch.float32)*0.5, torch.ones((raw_out.size()[0], raw_out.size()[1]), dtype=torch.float32)], dim=0).to(device)
            #         logits = logits.abs()
                
            #         optim.zero_grad()
            #         loss_i = F.binary_cross_entropy_with_logits(
            #             logits, occ, reduction='none')
            #         loss_occ = loss_i.sum(-1).mean()
            #         loss_occ.backward()
            #         optim.step()
        ...
        if self.BA:
            ...
            for id, frame in enumerate(optimize_frame):
                if frame != -1:
                    ...
                    cur_c2w = c2w.clone()
        if self.BA:
            return cur_c2w, batch_rays_d, batch_rays_o, batch_gt_depth
        else:
            return  None, batch_rays_d, batch_rays_o, batch_gt_depth
    ...
    def run(self):
        ...
        while (1):
            ...
            for outer_joint_iter in range(outer_joint_iters):
                ...
                _, batch_rays_d, batch_rays_o, batch_gt_depth = self.optimize_map(num_joint_iters, lr_factor, idx, gt_color, gt_depth,
            ...
            if self.SA:
                for i in range(1):
                    ...
                    for iter in range(0, 5): # 10
                        raw, raw_out = self.renderer.render_batch_ray(self.c, self.decoders, batch_rays_d,
                                                    batch_rays_o, self.device, stage,
                                                    gt_depth= batch_gt_depth, sa = True)
                        
                        logits = torch.cat([raw, raw_out], dim=1)
                        occ = torch.cat([torch.ones((1, raw.size()[1]), dtype=torch.float32)*0.5, torch.ones((1, raw_out.size()[1]), dtype=torch.float32)], dim=1).to(self.device)
                        # occ = torch.cat([torch.ones((raw.size()[0], raw.size()[1]), dtype=torch.float32)*0.5, torch.ones((raw_out.size()[0], raw_out.size()[1]), dtype=torch.float32)], dim=0).to(device)
                        logits = logits.abs()
        ...
        if self.coarse:
            ...
            for key, val in ckpt['model'].items():
                if ('decoder' in key) and ('encoder' not in key):
                    key = key[8:]
                    coarse_dict[key] = val
            self.decoders.coarse_decoder.load_state_dict(coarse_dict)
        ...
        for key, val in ckpt['model'].items():
            if ('decoder' in key) and ('encoder' not in key):
                if 'coarse' in key:
                    ...
                    key = key[8+5:]
                    fine_dict[key] = val
        self.decoders.middle_decoder.load_state_dict(middle_dict)
        self.decoders.fine_decoder.load_state_dict(fine_dict)
