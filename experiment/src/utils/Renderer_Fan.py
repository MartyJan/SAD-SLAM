...
class Renderer(object):
    ...
    def eval_points(self, p, decoders, c=None, stage='color', device='cuda:0', sa=False):
        """
        ...
        for pi in p_split:
            ...
            if sa:
                ...
                # ret_out = ret
                # ret = ret[~mask,-1]
                # ret_out = ret_out[mask,-1]
                # indices = torch.randperm(len(ret_out))[:300]
                # ret_out = ret_out[indices]
                # print('ret:',ret.shape)
                # print('ret_out:',ret_out.shape)
          
            rets.append(ret)
            # if sa:
            #     rets_out.append(ret_out)
        
        if sa:
            ret = torch.cat(rets, dim=0)
            # ret_out = torch.cat(rets_out, dim=0)
            ret = ret.unsqueeze(0)
            # ret_out = ret_out.unsqueeze(0)
            return ret
    ...
    def render_batch_ray(self, c, decoders, rays_d, rays_o, device, stage, gt_depth=None, sa=False):
        """
        ...
        if sa:
            noise = 0.1 * np.random.normal(0, 1.0, size=gt_depth.cpu().numpy().shape)
            noise = torch.from_numpy(noise).type(torch.FloatTensor).to(device)
            # print('noise',noise)
            out_depth = gt_depth + noise
            pts = rays_o[..., None, :] + rays_d[..., None, :] * gt_depth[...,None]
            pointsf = pts.reshape(-1, 3)
            raw = self.eval_points(pointsf, decoders, c, stage, device, sa = True)

            pts_out = rays_o[..., None, :] + rays_d[..., None, :] * out_depth[...,None]
            pointsf_out = pts_out.reshape(-1, 3)
            raw_out = self.eval_points(pointsf_out, decoders, c, stage, device, sa = True)
            indices = torch.randperm(len(raw_out.squeeze(0)))[:300]
            raw_out = raw_out[:,indices]
            # print(raw.shape)
            # print(raw_out.shape)
            return raw, raw_out
