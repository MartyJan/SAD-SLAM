...
class Mapper(object):
    ...
    def __init__(self, cfg, args, slam, coarse_mapper=False
        ...
        self.K = np.array([[self.fx, .0, self.cx], [.0, self.fy, self.cy], [.0, .0, 1.0]]).reshape(3, 3)
    ...
    def optimize_map(self, num_joint_iters, lr_factor, idx, cur_gt_color, cur_gt_depth, gt_cur_c2w, keyframe_dict, keyframe_list, cur_c2w):
        ...
        for joint_iter in range(num_joint_iters):
            ...
            if self.handle_dynamic:
                ...
                depth_mask = (tmp < 10*tmp.median()) & (batch_gt_depth > 0)
            else:
                depth_mask = batch_gt_depth > 0
            """
    ...
    def unproject_points(self, uvs):
        ...
        return np.dot(np.linalg.inv(K), self.add_ones(uvs).T).T[:, 0:2] 

    def rescale_translation_factor(self, kp_ref_u, kp_cur_u):
        kpn_ref = self.unproject_points(kp_ref_u)
        kpn_cur = self.unproject_points(kp_cur_u)
    ...
    def run(self):
        ...
        while (1):
            ...
            if not init:
                ...
                output = model([transforms(pil_img)])
                output = output[0]     
                ...
                if len(person_masks) > 0:
                        ...
                        for h in range(bool_masks.shape[1]):
                            for w in range(bool_masks.shape[2]):
                                if bool_masks[i][h][w] == True:
                                    ...
                                    sum_color_b = 0
                                    sum_depth = 0
                                    for prevframe in range(len(self.keyframe_dict)-1):
                                        print(f"Inpainting frame {idx}: h={h}, w={w}")
                                        
                                        # Initiate ORB detector
                                        # orb = cv2.ORB_create()
                                        
                                        prev_color = self.keyframe_dict[prevframe]['color']
                                        prev_depth = self.keyframe_dict[prevframe]['depth']
                                        ...
                                        neighbor_t = neighbor_t * self.rescale_translation_factor(refPts, currPts)

                                        prev_color_img = np.transpose(prev_color_img, (1,0,2))  # [h, w, c] -> [w, h ,c]
                                        # curr_color_img = np.transpose(curr_color_img, (1,0,2)) 
                                        prev_depth_img = np.transpose(prev_depth.detach().cpu().numpy(), (1,0))
                                        curr_depth_img = np.transpose(gt_depth.detach().cpu().numpy(), (1,0))
                                        ...
                                        try: # Handle out of bound exception
                    ...
                    gt_color = torch.from_numpy(curr_color_img / 255.).permute(1,0,2).to(self.device) # .permute(1,0,2)
                    gt_depth = torch.from_numpy(curr_depth_img).permute(1,0).to(self.device)
                
                # cv2.imwrite(f'./img/{idx}_color.png', gt_color.detach().cpu().numpy()*255.)
                # cv2.imwrite(f'./img/{idx}_depth.png', gt_depth.detach().cpu().numpy()*self.png_depth_scale)
