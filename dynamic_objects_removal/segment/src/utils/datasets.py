...
class TUM_RGBD_backup(BaseDataset):
	def __init__(self, cfg, args, scale, device='cuda:0'
				 ):
		super(TUM_RGBD, self).__init__(cfg, args, scale, device)
		self.color_paths, self.depth_paths, self.poses = self.loadtum(
...
class RS_RGBD():
    def __init__(self, cfg, args, scale, device='cuda:0'
        ...
        self.input_folder = os.path.join(self.input_folder)
        self.color_paths = sorted(
            glob.glob(os.path.join(self.input_folder, 'output_rgb', '*.png')))
        self.depth_paths = sorted(
            glob.glob(os.path.join(self.input_folder, 'output_depth', '*.npy')))
        self.n_img = len(self.color_paths)
        self.load_poses(os.path.join(self.input_folder, 'trajectories'))
    ...
    def __getitem__(self, index):
        # print("self.color_paths[index]: ", self.color_paths[index])
        # print("self.color_paths[index]: ", self.input_folder, '\t', self.color_paths)
        # print("index", index)
        color_path = self.color_paths[index]
        depth_path = self.depth_paths[index]
        color_data = cv2.imread(color_path)
