...
def check_proj(points, W, H, fx, fy, cx, cy, c2w):
    ...
    c2w[:3, 1] *= -1.0
    c2w[:3, 2] *= -1.0
    points = torch.from_numpy(points).clone()
    w2c = np.linalg.inv(c2w)
    w2c = torch.from_numpy(w2c).float()
    K = torch.from_numpy(
        np.array([[fx, .0, cx], [.0, fy, cy], [.0, .0, 1.0]]).reshape(3, 3))
    ones = torch.ones_like(points[:, 0]).reshape(-1, 1)
    homo_points = torch.cat(
        [points, ones], dim=1).reshape(-1, 4, 1).float()  # (N, 4)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--rec_mesh', type=str, default = 'output/room0_sa2/mesh/final_mesh_eval_rec.ply',
                        help='reconstructed mesh file path')
    parser.add_argument('--gt_mesh', type=str,default = 'room0.ply',
